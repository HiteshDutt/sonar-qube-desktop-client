...
function applyCliOverrides(args: Record<string, string | boolean>): void {
    ...
    if (cli.parallelReadCalls)      appsettings.parallelReadCalls    = parseInt(cli.parallelReadCalls, 10);
    if ('requireParallelRuleRead' in args)
        appsettings.requireParallelRuleRead = cli.requireParallelRuleRead === true || cli.requireParallelRuleRead === 'true';
}
